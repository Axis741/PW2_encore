...
function Home() {
    ...
    const handleSeleccionarProducto = async (idProducto) => {
        ...
        if(producto){
            ...
            if(resVariantes?.success){
                const variantesDB = resVariantes.data;
                setVariantes(variantesDB);

                const tallasDB = variantesDB
                .filter(v => v.talla)
                .map(v => v.talla);
                ...
                variantesDB.forEach((v) => {
                    if(v.talla){
                        stockObj[v.talla] = Number(v.stock);
                    }else{
                        setStock(Number(v.stock));
                    }
                });
                console.log(stockObj);
                setStockTallas(stockObj);
            }
        }
    }
    ...
    const handleActualizarProducto = async (e) => {
        ...
        const variantesActualizadas = tallas.map((talla) => ({
            talla,
            stock: Number(stockTallas[talla]) || 0
        }));
        ...
        const res = await updateProducto(productoSeleccionado, formData);
        if(res?.success){
            setMensajeVisible("Producto Actualizado");
            setShowEditModal(false);
            limpiarDatos();
            cargarProductos();
        }
    ...
    const handleEliminarProducto = async (e) => {
        e.preventDefault();
        const res = await deleteProducto(productoSeleccionado);
        if(res?.success){
            setMensajeVisible("Producto Eliminado");
            setShowDeleteModal(false);
            limpiarDatos();
            cargarProductos();
        }else{
    ...
    {showEditModal && (
        <section class="admin-container">
            ...
            <div class="admin-card">
                ...
                <form id="productForm" onSubmit={handleActualizarProducto}>
                    <div className='input-group'>
                        <label> Seleccionar Producto</label>
                        <select value={productoSeleccionado} onChange={(e) => handleSeleccionarProducto(e.target.value)}>
                            <option value="">Selecciona un producto</option>
                            {productos.map((producto) =>(
                                <option key={producto._id} value={producto._id}>{producto.nombre_producto}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div class="input-group">
                        <label>Imagen del producto</label>
                        <input type="file" name="imagen" accept="image/*" onChange={handleImageChange} ref={fileInputeRef}/>
                    </div>
                    ...
                    <button type="submit" className="btn_subir">Guardar Cambios</button>
    ...
    {showDeleteModal && (
        <section class="admin-container">
            ...
            <div class="admin-card">
                ...
                <form id="productForm" onSubmit={handleEliminarProducto}>
                    <div className='input-group'>
                        <label> Seleccionar Producto</label>
                        <select value={productoSeleccionado} onChange={(e) => handleSeleccionarProducto(e.target.value)}>
                            <option value="">Selecciona un producto</option>
                    ...
                    <button type="submit" className="btn_subir">Eliminar Producto</button>
